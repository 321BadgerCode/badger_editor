...
public class box_2 : Form
{
	...
	public box_2(string title1, string text1, Size? size1) { init(); setup(title1, new text_box(text1, new style2(null, null, null)),size1); }
	public box_2(string title1, text_box text1, Size? size1) { init(); setup(title1, text1, size1); }
	public box_2(string title1, string text1) { init(); setup(title1, new text_box(text1, new style2(null, null, null)), new Size(400,250)); }
	public box_2(string title1, text_box text1) { init(); setup(title1, text1, new Size(400,250)); }
	...
	public void error() { t = type.message; audio_1.play(SystemSounds.Exclamation); icon = SystemIcons.Error; build(); }
	...
	public string input() { t = type.input; icon = SystemIcons.Application; input_box = new TextBox(); AutoCompleteStringCollection acsc=new AutoCompleteStringCollection(); acsc.AddRange(language.s.keyword); input_box.AutoCompleteCustomSource = acsc; input_box.AutoCompleteMode = AutoCompleteMode.SuggestAppend; input_box.AutoCompleteSource = AutoCompleteSource.CustomSource; build(); return input_box.Text; }
};
//classes for various items/controls like the text_box class for things, such as the slider
//list/array like new control[]{} with all of controls that display in box
//allow option to not allow user to move form("setBounds(this.X,this.Y);")